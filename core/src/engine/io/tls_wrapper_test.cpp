...
constexpr auto chain_primary_cert = R"(-----BEGIN CERTIFICATE-----
...
// intermediate certificate for other_key and other_cert was generated by
// openssl x509 -req -in <(openssl req -new -key testing_priv.key -subj "/CN=intermediate_test") -CA testing_cert.crt -CAkey testing_priv.key -CAcreateserial -out intermediate_cert.crt -days 1825 -sha256
// full chain generated by
// cat intermediate_cert.crt testing_cert.crt > fullchain.crt
...
UTEST(TlsWrapper, ConnectTimeout) {
  ...
  EXPECT_THROW(static_cast<void>(io::TlsWrapper::StartTlsServer(
                   std::move(server), crypto::LoadCertficatesChainFromString(cert),
