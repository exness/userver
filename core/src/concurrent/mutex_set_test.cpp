...
UTEST(MutexSet, MultipleKeys) {
    ...
    const std::unique_lock lock1(m1);
    const std::unique_lock lock2(m2);
}
...
UTEST(MutexSet, Sample) {
    ...
    {
        const std::unique_lock lock_first(m1);
        const std::unique_lock lock_second(m2);
    ...
    const std::unique_lock lock(m1_again);
    /// [Sample mutex set usage]
}
