...
__attribute__((optnone)) static void test_non_coroutine_ctx() {
...
__attribute__((optnone)) static void test_single_coroutine() {
    engine::RunStandalone([&] {
        void* volatile current_task = &engine::current_task::GetCurrentTaskContext();
...
__attribute__((optnone)) static void test_sleeping_coroutine(bool no_span = false) {
    engine::RunStandalone([&] {
        void* volatile root_coro = &engine::current_task::GetCurrentTaskContext();
        auto root_coro_name = tracing::Span::CurrentSpan().GetName();
        auto root_stacktrace = boost::stacktrace::stacktrace();
        auto payload = [&] {
            void* volatile new_coro = &engine::current_task::GetCurrentTaskContext();
...
__attribute__((optnone)) static void test_multiple_coroutines(int tasks_cnt = 11, int threads = 1) {
...
__attribute__((noinline)) void benchmark_heavy_service(size_t tasks_cnt, size_t threads_cnt, size_t memory_per_task) {
    engine::RunStandalone(threads_cnt, [&] {
        auto payload = [&](size_t index, auto self) -> void {
            ...
            if (index == tasks_cnt) {
                ...
                utils::Async("task_" + std::to_string(index + 1), self, index + 1, self).Wait();
            }
            DoNotOptimize(some_used_memory);
        };
        utils::Async("task_1", payload, 1, payload).Wait();
    });
}

void run_tests() {
    test_non_coroutine_ctx();
    test_single_coroutine();
    test_sleeping_coroutine();
    test_sleeping_coroutine(true);
    test_multiple_coroutines();
    test_multiple_coroutines(100, 8);
    test_multiple_coroutines(1000, 32);
}

void run_benchmarks() {
    benchmark_heavy_service(100, 8, 1'000);
    benchmark_heavy_service(100, 8, 10'000'000);
    benchmark_heavy_service(500, 32, 1'000'000);
    benchmark_heavy_service(1000, 32, 100'000);
    benchmark_heavy_service(2000, 32, 100'000);
}

USERVER_NAMESPACE_END

int main() {
    USERVER_NAMESPACE::run_tests();
    USERVER_NAMESPACE::run_benchmarks();
}
